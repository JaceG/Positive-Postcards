...
// Check if Stripe secret key is available (only required at runtime, not build time)
if (!process.env.STRIPE_SECRET_KEY) {
	console.warn(
		'WARNING: STRIPE_SECRET_KEY is not set in environment variables'
	);
	// Only exit if we're actually trying to start the server (not during build)
	if (process.argv.includes('server.js')) {
		console.error(
			'Please ensure you have STRIPE_SECRET_KEY set for production'
		);
		process.exit(1);
	}
}
...
const requireStripe = (req, res, next) => {
	if (!stripe) {
		return res.status(503).json({
			error: 'Payment service is temporarily unavailable. Please ensure Stripe is properly configured.',
...
app.get('/api/prices', requireStripe, async (req, res) => {
	try {
...
app.post('/api/create-portal-session', requireStripe, async (req, res) => {
	try {
		const { customerId } = req.user;
...
app.post('/api/subscription/pause', requireStripe, async (req, res) => {
	try {
...
// Resume subscription
app.post('/api/subscription/resume', requireStripe, async (req, res) => {
	try {
		const { subscriptionId } = req.body;
